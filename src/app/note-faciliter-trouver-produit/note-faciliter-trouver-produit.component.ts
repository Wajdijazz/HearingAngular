...
import { NoteChoixProduitService } from '../note-choix-produit/note-choix-produit.service';
...
@Component({
  ...
  templateUrl: './note-faciliter-trouver-produit.component.html',
  styleUrls: ['./note-faciliter-trouver-produit.component.scss']
})
export class NoteFaciliterTrouverProduitComponent implements OnInit {
  id_societe: number;
  societe: Societe[];
  nom_Societe: String;
...
TotalReponse=result.length
 ...
 if (index1=== -1){
   this.NoteFaciliteTrouverProduitEnseigne.push({ label: element+"-"+year, y:Math.round(ScoreTheme)}) 
 }
 })
 this.lineChart1( this.nom_Societe,this.NoteFaciliteTrouverProduitEnseigne,"chartupperleft")
...
this.concurrentService.getAlConcurrent(this.id_societe).subscribe((data:Concurrent[])=>{
  this.FaciliteTrouverProduitConcurrent= data.filter((word =>word.Facilite_trouver_produits_concurrent != "") )
   ...
   var tab_concurrent=[]
   var i=0
   this.FaciliteTrouverProduitConcurrent.forEach(concurrent=>{
...
var year = yearTime.getFullYear()
 ...
 monthNames.forEach(element=>{
   tab_concurrent.forEach(ta=>{
     ...
     AMN=0
     P=0
var res=this.FaciliteTrouverProduitConcurrent=data.filter((word => monthNames[new Date(word.date_reponse_concurrent).getMonth()]==element && word.concurrent==ta.concurrent && word.Facilite_trouver_produits_concurrent != "")) 
 ...
 res.forEach(el=>{  
   var d = new Date(el.date_reponse_concurrent)
    ...
    if(dateTime==element){
     ...
     if(el.Facilite_trouver_produits_concurrent=="plus facile"){
        M=M+1

      }
       if(el.Facilite_trouver_produits_concurrent=="aussi facile"){
         AMN=AMN+1
       }
...
this.lineChart1("",this.Month1,"chartbottomright");
  ...
  onSelected(pointevente): void{
    ...
    TotalReponse=result.length
     ...
     var index =  this.NoteFaciliteTrouverProduitMagasin.findIndex(x => x.label==element+"-"+year)
      if (index === -1){
        this.NoteFaciliteTrouverProduitMagasin.push({ label: element+"-"+year, y:Math.round(ScoreTheme)}) 
      }
     
      })
   this.lineChart1(pointevente,this.NoteFaciliteTrouverProduitMagasin,"chartbottomleft");
  ...
  this.NoteFaciliteTrouverProduitMagasinConcurrent.push({ label: element+"-"+year, y:Math.round(ScoreRelatif)}) 
  })
  this.lineChart1(concuurent,this.NoteFaciliteTrouverProduitMagasinConcurrent,"chartbottomright");  
