...
export class NoteQualiteProduitsComponent implements OnInit {
  ...
  magasins=[]
  concurrents=[]
  Qualiteproduit: ReponsePointeVente[];
  NoteQualiteProduitEnseigne=[]
  QualiteProduitConcurrent: Concurrent[];
  concurrent1: any;
  concurrent2: any;
  concurrent3: any;
  concurrent4: any;
  ...
  constructor(private pointeventereponseService:PointeventereponseService ,private userService:UserService,private zone: NgZone,private concurrentService: ConcurrentService ,  private pointeventeService: PointVenteService, private societeService :SocieteService, private qualiteProduitService:QualiteProduitService, private pointventeBySocieteService : PointventeBySocieteService, private dialog:MatDialog) { }
    ...
    this.societeService.getSocieteById(this.id_societe).subscribe((data:Societe[])=>{
      ...
      this.societe.forEach(element=>{
        ...
        this.pointeventereponseService .getReponseEnseigne(this.id_societe).subscribe((data:ReponsePointeVente[])=>{
          var datapointevenete=data.filter(word =>  word. Qualite_Produit_satisfaction!= "");
...
if (index === -1){
 ...
 monthNames.forEach(element=>{
    ...
    PTS=0
    PDTS=0
const result = this.Qualiteproduit.filter(word => monthNames[new Date(word.date_reponse_pointevente).getMonth()]==element && word.Qualite_Produit_satisfaction !="");
...
TotalReponse=result.length
 ...
 this.lineChart1(this.nom_Societe,this.NoteQualiteProduitEnseigne,"chartupperleft")
...
this.lineChart1("",this.Month1,"chartbottomright");
  ...
  onSelected(pointevente): void{
    ...
    this.pointeventereponseService.getReponseMagasin(this.id_societe,pointevente).subscribe((data:ReponsePointeVente[])=>{
      ...
      monthNames.forEach(element=>{
         ...
         PTS=0
         PDTS=0
    const result = data.filter(word => monthNames[new Date(word.date_reponse_pointevente).getMonth()]==element && word.Qualite_Produit_satisfaction !="");
    var yearTime=new Date()
    var year = yearTime.getFullYear()
    TotalReponse=result.length
      result.forEach(el=>{  
   ...
   this.lineChart1(pointevente,this.NoteQualiteProduitMagasin,"chartbottomleft");
  ...
  onSelectedConcurrent(concuurent){
    ...
    this.concurrentService.getMagasinConcurrent(this.id_societe,this.selectedpointvenete,concuurent).subscribe((data:Concurrent[])=>{
         var QualiteProduitConcurrentMagasin= data.filter((word =>word.Qualite_Produit_concurrent != "") )
  ...
  monthNames.forEach(element=>{
    ...
    AMN=0
    P=0 
  const result = QualiteProduitConcurrentMagasin.filter(word => monthNames[new Date(word.date_reponse_concurrent).getMonth()]==element);
  console.log(result)
  var yearTime=new Date()
  var year = yearTime.getFullYear()
  TotalReponse=result.length
  ...
  this.lineChart1(concuurent,this.NoteQualiteProduitMagasinConcurrent,"chartbottomright");  
