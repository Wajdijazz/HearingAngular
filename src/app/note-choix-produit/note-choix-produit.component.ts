...
this.lineChart1("En relatif vs la concurrence","",this.Month1,"chartbottomright");
  ...
  onSelected(Idselected): void{
    ...
    this.choixProduitsService.getChoixProduitsMagasin(Idselected,this.id_societe).subscribe((data:NoteChoixProduitReponse[])=>{
  
      this.ChoixProduitsMagasin=data
        var TS
        var AS
        var PTS
        var PDTS
      ...
      monthNames.forEach(element=>{
         ...
         PTS=0
         PDTS=0
    const result = this.ChoixProduitsMagasin.filter(word => monthNames[new Date(word.date_reponse).getMonth()]==element);
    var yearTime=new Date()
    var year = yearTime.getFullYear()
    TotalReponse=result.length
      result.forEach(el=>{  
   ...
   this.lineChart1("Evolution de la Choix produit Magasin",nom_selected_point_vente,this.NoteChoixProduitsMagasin,"chartbottomleft");
