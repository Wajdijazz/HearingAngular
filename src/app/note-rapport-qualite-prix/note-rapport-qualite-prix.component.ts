...
import { NoteRapportQualitePrixConcurrent } from './note-rapport-qualite-prix-concurrent';
import { NoteRapportQualitePrixReponse } from './note-rapport-qualite-prix-reponse';
...
@Component({
  selector: 'app-note-rapport-qualite-prix',
  templateUrl: './note-rapport-qualite-prix.component.html',
  styleUrls: ['./note-rapport-qualite-prix.component.scss']
})
export class NoteRapportQualitePrixComponent implements OnInit {
  id_societe: number;
  societe: Societe[];
  nom_Societe: String;
  ...
  magasins=[]
  concurrents=[]
  RapportQualitePrix: NoteRapportQualitePrixReponse[];
  NoteRapportQualitePrixEnseigne=[]
  rapportQualitePrixConcurrent: NoteRapportQualitePrixConcurrent[];
  concurrent1: any;
  concurrent2: any;
  concurrent3: any;
  concurrent4: any;
  ...
  RapportQualitePrixMagasin: NoteRapportQualitePrixReponse[];
  NoteRapportQualitePrixMagasin=[]
  Month=[]
  Month1=[]
  NoteRapportQualitePrixMagasinConcurrent=[]
  userInfo: { id: any; id_societe: any; name: any; email: any; };
  selectedpointvenete: any;

  constructor(private pointeventereponseService:PointeventereponseService,private userService:UserService,private zone: NgZone,private concurrentService: ConcurrentService ,  private pointeventeService: PointVenteService, private societeService :SocieteService, private rapportqualieprixService:NoteRapportQualitePrixService, private pointventeBySocieteService : PointventeBySocieteService, private dialog:MatDialog) { }
      ...
      this.societe.forEach(element=>{
        this.nom_Societe=element.nom
             ...
             this.pointeventereponseService .getPointeventeName().subscribe((data:ReponsePointeVente[])=>{
              var datapointevenete=data.filter(word => word.id_societe==this.id_societe && word. Rapport_qualite_prix_satisfaction!= "");
...
this.concurrentService.getConcurrent().subscribe((data:NoteRapportQualitePrixConcurrent[])=>{
  var res=data.filter((word =>word.Rapport_qualite_prix_concurrent != "" && word.id_societe==this.id_societe) )
...
/******************************************************* Rapport Qualité Prix enseigne et Calcul **************************************************/

this.rapportqualieprixService.getRapportQualitePrix(this.id_societe).subscribe((data1:NoteRapportQualitePrixReponse[])=>{
        
  this.RapportQualitePrix =data1
  console.log(this.RapportQualitePrix)
 ...
 monthNames.forEach(element=>{
    ...
    PTS=0
    PDTS=0
const result = this.RapportQualitePrix.filter(word => monthNames[new Date(word.date_reponse).getMonth()]==element);
...
TotalReponse=result.length
 ...
 if (index1=== -1){
   this.NoteRapportQualitePrixEnseigne.push({ label: element+"-"+year, y:Math.round(ScoreTheme)}) 
 }
 })
 this.lineChart1("Evolution de la rapport qualité prix Enseigne", this.nom_Societe,this.NoteRapportQualitePrixEnseigne,"chartupperleft")

})



/******************************************************* Rapport Qualité Prix concurrent enseigne et Calcul **************************************************/

this.rapportqualieprixService.getRapportQualitePrixConcurrentSociete(this.id_societe).subscribe((data:NoteRapportQualitePrixConcurrent[])=>{

  this.rapportQualitePrixConcurrent=data.filter((word =>word.Rapport_qualite_prix_concurrent != "") )

   var tab_concurrent=[]
   var i=0
   this.rapportQualitePrixConcurrent.forEach(concurrent=>{
...
var year = yearTime.getFullYear()
 ...
 monthNames.forEach(element=>{
   tab_concurrent.forEach(ta=>{
     ...
     AMN=0
     P=0
var res=this.rapportQualitePrixConcurrent=data.filter((word => monthNames[new Date(word.date_reponse_concurrent).getMonth()]==element && word.concurrent==ta.concurrent && word.Rapport_qualite_prix_concurrent != "") )
 ...
 res.forEach(el=>{  
   var d = new Date(el.date_reponse_concurrent)
    ...
    if(dateTime==element){
     ...
     if(el.Rapport_qualite_prix_concurrent=="meilleure"){
        M=M+1

      }
      if(el.Rapport_qualite_prix_concurrent=="au même niveau de rapport qualité prix"){
        AMN=AMN+1
       }
...
this.lineChart("En relatif vs la concurrence",this.concurrent1,this.concurrent2,this.concurrent3,this.concurrent4, this.chartdata,"chartupperright")
 ...
 this.lineChart1("Evolution de la rapport qualité prix Magasin","",this.Month,"chartbottomleft");
...
this.lineChart1("En relatif vs la concurrence","",this.Month1,"chartbottomright");
  ...
  onSelected(pointevente): void{
    ...
    this.rapportqualieprixService.getRapportQualitePrixMagasin(this.id_societe,pointevente).subscribe((data:ReponsePointeVente[])=>{
      ...
      monthNames.forEach(element=>{
         ...
         PTS=0
         PDTS=0
    const result = data.filter(word => monthNames[new Date(word.date_reponse_pointevente).getMonth()]==element && word.Rapport_qualite_prix_satisfaction!="");
    var yearTime=new Date()
    var year = yearTime.getFullYear()
    TotalReponse=result.length
      result.forEach(el=>{  
     ...
     var index =  this.NoteRapportQualitePrixMagasin.findIndex(x => x.label==element+"-"+year)
      if (index === -1){
        this.NoteRapportQualitePrixMagasin.push({ label: element+"-"+year, y:Math.round(ScoreTheme)}) 
      }
     
      })
   this.lineChart1("Evolution de la rapport qualité prix Magasin",pointevente,this.NoteRapportQualitePrixMagasin,"chartbottomleft");
      this.NoteRapportQualitePrixMagasin=[]
    })
  ...
  this.rapportqualieprixService.getRapportQualitePrixConcurrentMagasin(this.selectedpointvenete,concuurent).subscribe((datac:NoteRapportQualitePrixConcurrent[])=>{
    var RapportQualitePrixConcurrentMagasin= datac.filter((word =>word.Rapport_qualite_prix_concurrent != "") )
  ...
  monthNames.forEach(element=>{
    ...
    AMN=0
    P=0 
  const result = RapportQualitePrixConcurrentMagasin.filter(word => monthNames[new Date(word.date_reponse_concurrent).getMonth()]==element && word.id_societe==this.id_societe);
  var yearTime=new Date()
  var year = yearTime.getFullYear()
  TotalReponse=result.length
  result.forEach(el=>{  
  ...
  this.NoteRapportQualitePrixMagasinConcurrent.push({ label: element+"-"+year, y:Math.round(ScoreRelatif)}) 
  })
  this.lineChart1("En relatif vs la concurrence",concuurent,this.NoteRapportQualitePrixMagasinConcurrent,"chartbottomright");  
    this.NoteRapportQualitePrixMagasinConcurrent=[]
  })
