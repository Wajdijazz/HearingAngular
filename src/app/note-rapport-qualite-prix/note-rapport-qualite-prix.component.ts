...
export class NoteRapportQualitePrixComponent implements OnInit {
  ...
  constructor(private userService:UserService,private zone: NgZone,private concurrentService: ConcurrentService ,  private pointeventeService: PointVenteService, private societeService :SocieteService, private rapportqualieprixService:NoteRapportQualitePrixService, private pointventeBySocieteService : PointventeBySocieteService, private dialog:MatDialog) { }
...
this.lineChart1("En relatif vs la concurrence","",this.Month1,"chartbottomright");
  ...
  onSelected(Idselected): void{
    ...
    this.rapportqualieprixService.getRapportQualitePrixMagasin(Idselected,this.id_societe).subscribe((data:NoteRapportQualitePrixReponse[])=>{
  
      this.RapportQualitePrixMagasin=data
        var TS
        var AS
        var PTS
        var PDTS
      ...
      monthNames.forEach(element=>{
         ...
         PTS=0
         PDTS=0
    const result = this.RapportQualitePrixMagasin.filter(word => monthNames[new Date(word.date_reponse).getMonth()]==element);
    var yearTime=new Date()
    var year = yearTime.getFullYear()
    TotalReponse=result.length
      result.forEach(el=>{  
   ...
   this.lineChart1("Evolution de la rapport qualité prix Magasin",nom_selected_point_vente,this.NoteRapportQualitePrixMagasin,"chartbottomleft");
  ...
  this.rapportqualieprixService.getRapportQualitePrixConcurrentMagasin(this.id_selectedpointvenete,concuurent).subscribe((datac:NoteRapportQualitePrixConcurrent[])=>{
